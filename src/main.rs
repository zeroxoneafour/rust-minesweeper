...
fn main() {
    let height: u32 = 16;
    let width: u32 = 30;
    let mut minesweeper = Game::new(width, height);
    let initial_cursor = minesweeper.resize(terminal::size().unwrap().0, terminal::size().unwrap().1);
    let mine_count: u32 = ((height * width) as f64 / 5.0).floor() as u32;
    println!("generating map with size {}x{} and up to {} mines", width, height, mine_count);
